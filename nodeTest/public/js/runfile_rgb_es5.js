...
function fade_up_then_down() {
  var max = 255;
  var min = 0;
  storageArray.add({ 0: max, 1: parseInt(max / 2), 2: max }, sequenceDuration / 3).delay(sequenceDuration / 3).add({ 0: min, 1: min, 2: min }, sequenceDuration / 3)
  //Run the sequence
  .run(universe);
}
...
function randomIntensityChanges() {
  ...
  storageArray2.add(arrayObject, randomDuration()).delay(5000 - randomDuration).run(universe);
}
